...
export async function addAdDataMatch(_: any, { adID, input }: any) {
  ...
  if (matchAfter)
    return new Error(
      "A match starts after the new match. You may not alter time. Your inputBegins was: " +
        input.begins
    );
  ...
  if (new Date(input.begins).getTime() > new Date(input.ends).getTime())
    return new Error("Your inputBegins is after your inputEnds");

  // Check if the time span between the new input begins and ends is valid. It is valid if it is less than or equal to a day.
  let timeSpan =
    new Date(input.ends).getTime() - new Date(input.begins).getTime();
  if (timeSpan > 86400000)
    return new Error(
      "The time span between the new match begins and ends is greater than a day"
    );
