...
// !! Historical data shows how high it was at the time,, not how many it got in that period

"use strict";
const mongoose = require("mongoose");
const hm = require("./history-model");
const async = require("async");
const { default: createIntervaledTime } = require("../../../../time/interval");
...
module.exports = function historyPlugin(schema, options) {
  ...
  function processUpdate(next) {
    ...
    hm.HistoryModel(
      ...
      .findOne({ t: historyDoc.t, contentID: historyDoc.d.contentID })
      .then((doc) => {
        //  console.log(doc);
        ...
        if (doc) {
          hm.HistoryModel(
            ...
            .findOneAndUpdate(
              { t: historyDoc.t, contentID: historyDoc.d.contentID },
              {
                $set: {
                  t: historyDoc.t,
                  contentID: historyDoc.d.contentID,
                  d: {
